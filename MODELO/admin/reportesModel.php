...
//$sql="SELECT * FROM ventas WHERE MONTH(fecha) = '".$mes."' and YEAR(fecha) = '".$anio."' and iddepartamento='".$departamento."'";
 ...
 function getReportes(){
   ...
   $sql="SELECT * FROM ventas WHERE YEAR(fecha) = 2021";
 ...
 function getmonto1(){
    ...
    $conect->set_charset("utf8");
   //$sql="SELECT * FROM ventas WHERE YEAR(fecha) = 2020";
   $sql="SELECT SUM(monto) as suma FROM ventas WHERE YEAR(fecha) = 2021";
   $getUs=mysqli_query($conect,$sql);
    if($getUs->num_rows >0){
        while($row=mysqli_fetch_array($getUs)){
            ?>
<div class="col-sm-6 offset-sm-3">
 ...
 function getmonto2(){
    ...
    $conect->set_charset("utf8");
   //$sql="SELECT * FROM ventas WHERE YEAR(fecha) = 2020";
   $sql="SELECT SUM(monto) as suma FROM ventas WHERE YEAR(fecha) = 2020";
   $getUs=mysqli_query($conect,$sql);
    if($getUs->num_rows >0){
        while($row=mysqli_fetch_array($getUs)){
            ?>
<div class="col-sm-6 offset-sm-3">
 ...
 function getcantidad(){
    ...
    $conect->set_charset("utf8");
   //$sql="SELECT * FROM ventas WHERE YEAR(fecha) = 2020";
   $sql="SELECT SUM(cantidad_prod) as suma FROM ventas WHERE YEAR(fecha) = 2021";
   $getUs=mysqli_query($conect,$sql);
    if($getUs->num_rows >0){
        while($row=mysqli_fetch_array($getUs)){
            ?>
        <div class="col-sm-6 offset-sm-3">
            <div class="card-header bg-secondary">
              <a>Cantidad de productos vendidos: <?php  echo $row['suma']?></a>
            </div>
 ...
 function getcantidad2(){
    ...
    $conect->set_charset("utf8");
   //$sql="SELECT * FROM ventas WHERE YEAR(fecha) = 2020";
   $sql="SELECT SUM(cantidad_prod) as suma FROM ventas WHERE YEAR(fecha) = 2020";
   $getUs=mysqli_query($conect,$sql);
    if($getUs->num_rows >0){
        while($row=mysqli_fetch_array($getUs)){
            ?>
